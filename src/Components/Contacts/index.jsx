...
const Contacts = () => {
  ...
  const newContacts = contacts.map((item, index) => [
    ...
    item.address,
    item.jobName,
    `/dashboard/contacts/edit/${item._id}`,
  ]);
      ...
      options:
        {
          ...
          customBodyRender: (value, tableMeta) => (
            value
              ...
              : (
                <Avatar>
                  {tableMeta.rowData[3].slice(0, 1)
                    .toUpperCase()}
                  {tableMeta.rowData[4].slice(0, 1)
                    .toUpperCase()}
                </Avatar>
              )
          ),
  ...
  const handleRowClick = async (currentRowsSelected, allRowsSelected, rowsSelected) => {
    ...
    selected = rows.filter((element) => rowsSelected.includes(element.id))
