...
const declOfNumber = (number, words) => words[
...
const uploadImage = async (files, data) => {
  if (files.avatars !== null) {
    const fileExtension = files.avatars.name.split('.')
      .pop();
    // eslint-disable-next-line no-param-reassign
    data.avatars = `${data.id}.${fileExtension}`;
    const newPath = `${path.resolve(`${__dirname}/../uploaded/images/${data.id}`)
    }/${data.avatars}`;
...
const deleteImage = async (id) => {
  const deletePath = `${path.resolve(`${__dirname}/../uploaded/images/${id}`)}/`;
