...
router.post('/login', async (request, response) => {
  let data = await Users.findOne({email: request.body.email})
  if (data) {
    if (bcrypt.compareSync(request.body.password, data.password)) {
      ...
      response.json({result: 'success', token, message: 'Авторизация успешна'})
    } else {
      response.json({result: 'error', message: 'Неверный пароль'})
    }
  } else {
    response.json({
      result: 'error',
